...
function AssignTaskForm({handleAssignForm, data, reference, taskList}) {
  ...
  useEffect(() => {
    selectedUsers.map((user) => {
      setSelectedUsersId([user._id, ...selectedUsersId])
      // selectedUsersId.push(user._id)
    })
          ...
          selectedUsers.length > 0  && (
            ...
            >
              ...
              {selectedUsers.map((user) => (
                ...
                >{user.username}
                  <RxCross1 onClick={handleDelete} id={user._id} className="cursor-pointer" />
                </li>
              ))}
            </ul>
            </>
